...
gen_rows = function(n, names, modelo){
  names_param = modelo
  if(modelo != "ARMA_GARCH" && modelo != "SE"){
  ...
  if(modelo == "SE"){
    string = paste0("column(width = 2, numericInput('", names_param, 1,"', '",
                    names[1],"', value = 0, min = 0))", collapse = ",")
    string = paste0(string, ", column(width = 2, selectInput('", names_param, "2', label = h3('Erro'),
                    choices = list('A' =  1, 'M' = 2, 'Z' = 3), selected = 1)), 
                    column(width = 2, selectInput('",names_param,"3', label = h3('Trend'),
                    choices = list('A' =  1, 'M' = 2, 'Z' = 3, 'N' = 4), selected = 1)),
                    column(width = 2, selectInput('",names_param,"4', label = h3('Sazonalidade'),
                    choices = list('A' =  1, 'M' = 2, 'Z' = 3, 'N' = 4), selected = 1))")
  }
...
## ----tabela-----------------------------------------
### TABELA PRO SHINY
TabelaModelos <- matrix(ncol = 4,
                        c(" ", "AR-GARCH",   "Decomposição Multiplicativa", "MA-GARCH","MAE", 
                          "0,01604645", "0,01982128", "0,01604806","RMSE",
                          "0,02084067", "0,02961776", "0,02084361",
                          " ", " ", " ", " "),
                        byrow = T)
colnames(TabelaModelos) <- c(" ", " ", " ", " ")
TabelaModelos <- as.data.frame(TabelaModelos)


TabelaTestes <- matrix(ncol = 5, c("", "LRuc AR", "LRcc AR",
                            "LRuc MA", "LRcc MA", "Teste", "6,16963041","6,17995301",
                            "6,16963041", "6,17995301", "P-valor", "0,01299618",
                            "0,04550302", "0,01299618", "0,04550302",
                            " ", " ", " ", " ", " "), byrow = T)
colnames(TabelaTestes) <- c(" ", " ", " ", " ", " ")
TabelaTestes <- as.data.frame(TabelaTestes)

TabelaES <- matrix(ncol = 3,
            c(" ", "MA-GARCH",   "AR-GARCH", "P-valor Bootstrap", "0,993494", "0,993704",
"P-valor", "0,9871423", "0.9871511",
 " ", " ", " "),
                        byrow = T)
colnames(TabelaES) <- c(" ", " ", " ")
TabelaES <- as.data.frame(TabelaES)
...
lista_params = list(c("AR", "I", "MA"), c("AR", "MA", "GARCH 1", "GARCH 2"), c("Sazonalidade"),
                    ...
                    c("P"), c("Sazonalidade"), c("P", "Sazonalidade"), c("Sazonalidade"))
...
get_graph_model = function(dados, modelo, params, distribution = "norm", h = 5){
  ...
  acf2 <- ggplot(acf2)+
    ...
    meu_tema+
    scale_x_discrete(breaks = seq(0,35, by=5))+
    labs(x = "Lag", y = "Valor da ACF dos quadrados")
  ...
  hist_res <- ggplot(aux, aes(x = res))+
    ...
    labs(title = "Histograma do Resíduo")+
    meu_tema+
    labs(x = "Valor do resíduo", y = "Contagem")
  ...
  layoutgraf <- plot_layout(
   ...
   plot_grid <- (plot_seq)/(qqplot+hist_res)/(acf+acf2+pacf)+layoutgraf
...
if(!exists("dados")){
  ...
  acf <- ggplot(acf)+
    ...
    theme_bw()+
    meu_tema+
    scale_x_discrete(breaks = seq(1,36, by=5), labels = seq(0,35, by=5))+
    labs(x = "Lag", y = "Valor da ACF")
...
.content-wrapper,
.right-side, .dygraph-legend, .selectize-input.full, option, option active, option selected,
.option, .option.active, .option.active.selected, .option.selected{
...
tab_introducao <- column(12,
                         h1("Introdução"),
                         p("Um grupo de investidores está interessado em diversificar os segmentos de mercado nos quais investem. Para isso, eles procuraram o nosso grupo de estudantes de Séries Temporais (ME607) da Unicamp a fim de obter um modelo capaz de capturar a dinâmica dos preços de retorno das ações da Microsoft na Bolsa de Valores de São Paulo (BOVESPA), cujo código é MSFT34.SA, desde 01/01/2013.", style = "color: white"),
                         br(),
                           p("Para isso, inicialmente, foi feita uma análise exploratória dos preços de fechamento e dos retornos através de gráficos sequenciais e das funções de autocorrelação e autocorrelação parcial. Na sequência, foram ajustados diversos modelos, os quais passaram por um diagnóstico e, aqueles que capturaram bem a dinâmica dos dados foram comparados a partir de uma validação cruzada via rolling window. Por fim, aquele que obteve a melhor performance foi colocado em produção, sendo utilizado para prever os retornos ao longo de 7 dias úteis (a bolsa de valores não abre durante os sábados e domingos).", style = "color: white"))

tab_contexto <- column(12,
                       h1("Contexto"),
                       p("O princípio da diversificação afirma que é preciso diversificar a carteira de investimentos, de forma a reduzir riscos e melhorar a probabilidade de obtenção de resultados positivos. Com isso em mente, um grupo de investidores, após realizar um estudo de mercado, concluiu que a Microsoft seria, potencialmente, uma boa empresa para se adquirir ações. Entretanto, a fim de tomar decisões mais assertivas e baseadas em dados, eles contataram o nosso grupo de Séries Temporais (ME607) da Unicamp em busca de um modelo capaz de capturar a dinâmica dos preços dessas ações desde 2013 .
", style = "color: white"))

tab_abordagem <- column(12,
                        h1("Abordagem Utilizada"),
                        p("Considerando a situação apresentada, primeiramente foi realizada uma análise exploratória dos preços ajustados de fechamento das ações, incluindo gráficos sequenciais e das funções de autocorrelação e autocorrelação parcial. Na sequência, o mesmo foi realizado para o preço de retorno ajustado, de forma a tornar possível uma compreensão melhor da série.
", style = "color: white"),
                        br(),
                        p("Em seguida, há um local em que o usuário pode escolher o modelo que deseja tentar ajustar aos dados, o qual apresenta gráficos que permitem a realização de uma análise dos resíduos. Posteriormente, os modelos que foram considerados melhores dentre todos os que foram testados por nós e que passaram pelo diagnóstico foram submetidos a uma validação cruzada com rolling-window, através da qual foi escolhido e retornado o modelo com melhor desempenho.
", style = "color: white"))
...
tab_serie_fechamento <- column(12,
                    ...
                    tabsetPanel(
                      tabPanel("Gráfico",
                               br(),
                               p("O gráfico abaixo apresenta a evolução do preço de fechamento das ações ajustado. Nota-se que havia uma tendência de crescimento forte até janeiro de 2022, seguida de uma queda ocorrida até um ano depois, quando iniciou novamente um período de crescimento.", style = "color: white"),
                               br(),
                               dygraphOutput("serie_grafico")
                      ),
                      tabPanel("ACF",
                               br(),
                               p("A função de autocorrelação dos preços ajustados está apresentada abaixo. A partir dela, percebe-se forte tendência, de forma que é fácil ver que a série não é estacionária.", style = "color: white"),
                               br(),
                               plotOutput("serie_acf")
                      ),
                      tabPanel("PACF",
                               br(),
                               p("A função de autocorrelação parcial dos preços ajustados é ilustrada abaixo. Com base nela, conclui-se que há uma autocorrelação parcial muito forte com o valor do tempo anterior, de modo que é necessário modelá-la.", style = "color: white"),
                               br(),
...
tab_serie_retorno <- column(12,
                    ...
                    tabsetPanel(
                      tabPanel("Gráfico",
                               br(),
                               p("O gráfico abaixo mostra a evolução histórica dos retornos da série. É possível observar que a média é constante, diferentemente do que ocorre com a variância.", style = "color: white"),
                               br(),
                               dygraphOutput("serie_grafico_r")
                      ),
                      tabPanel("ACF",
                               br(),
                               p("O gráfico abaixo mostra a função de autocorrelação dos preços de retorno. Nele podemos ver que não há tendência e que há autocorrelação com os valores dos tempos anteriores (alguns valores ultrapassam os limites da banda), sendo assim necessário modelá-los.", 
                                 style = "color: white"),
                               br(),
                      ...
                      tabPanel("ACF dos quadrados dos retornos",
                               br(),
                               p("Abaixo se encontra o gráfico da função de autocorrelação dos preços de retorno ao quadrado. Vemos que vários valores ultrapassam os limites da banda, sendo assim necessário modelá-los, provavelmente usando um modelo da família GARCH.", style = "color: white"),
                               br(),
                               plotOutput("serie_acf2_r")
                      ),
                      tabPanel("PACF",
                               br(),
                               p("O gráfico a seguir mostra a função de autocorrelação parcial dos preços de retorno. Vemos que alguns valores ultrapassam a banda, sendo assim necessário modelá-los.", style = "color: white"),
                               br(),
...
tab_modelos <- column(12,
                      h1("Modelos"),
                      p("Considerando que você pode ter interesse em modelar a série de retornos, abaixo temos uma seleção de modelos interativa! Escolha um tipo de modelo, seus parâmetros e, se necessario, sua distribuição. Clique em Avaliar Modelo para visualizar os gráficos que mostram o comportamento dos resíduos do modelo ajustado.", style = "color: white"),
                      fluidRow(
                        column(12,
                               radioGroupButtons("modelo_nome", h3("Escolha seu modelo"),
                      ...
                      h3("Escolha seus parâmetros e avalie o modelo"),
...
tab_modelo_escolhido <- column(12,
                               h1("Modelos Escolhidos"),
                               p("Foram ajustados os modelos apresentados na aba anterior para as séries de retorno e fechamento. Quando foi necessário incluir um período sazonal, foram testados os períodos 5 (uma semana útil), 10 (duas semanas úteis) e 20 (um mês útil), os quais foram julgados adequados. ", style = "color: white"),
                               br(),
                               p("Após o ajuste desses cerca de 80 modelos considerados (os quais estão disponíveis no arquivo analise_das_series.R), foi realizado o diagnóstico de cada um deles, considerando ACF e PACF dos resíduos, ACF dos quadrados deles e um qqplot com a distribuição postulada, quando necessário. Vale ressaltar que esses gráficos foram realizados sequencialmente, de modo que nem todos foram necessários para concluir que o modelo não era adequado.", style = "color : white"),
                               br(),
                               p("A partir dessa análise, apenas três modelos passaram pelo diagnóstico e, portanto, foram considerados e comparados em uma validação cruzada utilizando rolling window, que são:", style = "color : white"),
                               tags$ul(
                          tags$li("AR(1)-GARCH(1,2)", style = "color : white"),
                          tags$li("MA(1)-GARCH(1,2)", style = "color : white"),
                          tags$li("Decomposição Multiplicativa com período sazonal 10", style = "color : white")
)
)
                               
#### ABA VALIDAÇÃO ###############################################
tab_validacao <- column(12,
                        h1("Cross-Validation com Rolling Window"),
                        br(),
                        p("A fim de comparar os modelos adequadamente, foi realizada uma validação cruzada (cross-validation) utilizando rolling window, a qual considerou uma janela de tamanho 2000, de um total de 2777 valores observados. Dessa forma, foram realizados 777 ajustes de cada um dos três modelos, seguidos de uma previsão do retorno, VaR (1%) e ES (1%) do dia seguinte. No cálculo da previsão da decomposição, foi necessário ajustar um ARMA(1,2)-GARCH(1,1) para a tendência.", style = "color : white"),
                        br(),
                        p("A seguir estão apresentados os valores do erro absoluto médio (MAE) e da raiz do erro quadrático médio (RMSE) dos erro de previsão dos retornos. Com base neles, nota-se que a decomposição multiplicativa teve um desempenho muito abaixo dos demais, além do fato de o modelo AR(1)-GARCH(1,2) ter apresentado uma performance levemente superior - a diferença aparece na sexta casa decimal. Assim, percebeu-se que o modelo baseado em decomposição multiplicativa é pior do que os demais e, por isso, não foi considerado nas análises subsequentes.", style = "color : white"),
                        br(),
                        tags$style(HTML("
                            .center-table {
                              margin-left: auto;
                        ...
                        tableOutput("Tabela1"),
                        p("Posteriormente, foram realizados backtests para a VaR (1%) e para a ES (1%). Para a primeira, foram considerados os testes de cobertura incondicional de Kupiec (LRuc) e de cobertura condicional de Christoffesen (LRcc), cujos resultados estão abaixo. Note que, a um nível de significância de 1%, não há evidências para rejeitar a hipótese de que o modelo está bem ajustado em ambos os testes (isso vale tanto para o MA-GARCH quanto para o AR-GARCH). Além disso, o desempenho de ambos os modelos foi bastante semelhante.", style = "color : white"),
                        br(),
                        tableOutput("Tabela2"),
                        p("Por fim, foram realizados backtests para a ES (1%) utilizando o teste de Expected Shortfall de McNeil e Frey, cujos p-valores estão apresentados abaixo. A partir de sua análise, conclui-se que não há evidências para rejeitar a hipótese de que o déficit condicional em excesso tem média zero. Assim, esse é mais um indicativo de que os modelos captaram a dinâmica de variabilidade dos dados.", style = "color : white"),
                        br(), 
                        tableOutput("Tabela3"),
                        p("Com base na validação cruzada apresentada e nos testes realizado, conclui-se que os modelos AR(1)-GARCH(1,2) e MA(1)-GARCH(1,2) possuem desempenhos extremamente semelhantes e superiores à decomposição multiplicativa de sazonalidade 10, sendo que o primeiro é levemente melhor. Por isso, o modelo escolhido e que será colocado em produção é o AR(1)-GARCH(1,2), o qual será utilizado para predizer os retornos, VaR (1%) e ES (1%).", style = "color : white")
...
tab_conclusao <- column(12,
                        h1("Conclusão"),
                        br(),
                        p("Com base nas análises exploratórias que foram realizadas tanto para a série de fechamento ajustado quanto de retorno ajustado, esperava-se que o modelo que traria melhores resultados seria um da classe ARMA-GARCH. Após o ajuste de vários modelos para ambas as séries, e seus respectivos diagnósticos, obtiveram-se três modelos que pareciam ter captado bem a dinâmica dos dados, sendo dois deles dessa classe. Na sequência, foi realizada uma cross-validation, a qual confirmou que ambos possuíam performances muito superiores ao terceiro. Dessa forma, como o AR(1)-GARCH(1,2) apresentou um desempenho um pouco melhor na previsão dos retornos, ele foi escolhido para ser colocado em produção. Assim, ele será utilizado para predizer os retornos, VaR (1%) e ES (1%) ao longo dos próximos sete dias úteis.", style = "color : white"),
...
server <- function(input, output, session) {
  ...
  ##Tabelas da aba de Validação#######
  output$Tabela1 <- renderTable({TabelaModelos})
  output$Tabela2 <- renderTable({TabelaTestes})
  output$Tabela3 <- renderTable({TabelaES})
